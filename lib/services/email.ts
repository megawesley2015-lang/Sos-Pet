...
export async function notificarPrestadorAprovado(opts: {
  ...
  const html = wrap(
    `<h2>✅ Cadastro aprovado!</h2>
     <p>Olá, <strong>${nome}</strong>!</p>
     <p>Ótima notícia! O cadastro de <strong>${nomeNegocio}</strong> foi <strong>aprovado</strong> e já está visível na plataforma SOS Pet.</p>
...
export async function notificarParceiroAprovado(opts: {
  ...
  const html = wrap(
    `<h2>✅ Sua solicitação de parceria foi aprovada!</h2>
     ...
     <p>Obrigado pelo interesse no SOS Pet!</p>`,
    "#22c55e"
  );

  return sendEmail({
    to: opts.email,
    subject: "✅ Solicitação de parceria aprovada — SOS Pet",
    html,
...
export async function notificarParceiroRejeitado(opts: {
  ...
  const html = wrap(
    `<h2>Solicitação de parceria não aprovada</h2>
     ...
     <p>Isso pode mudar no futuro conforme o SOS Pet evolui — se quiser saber mais ou esclarecer algo, basta responder este email.</p>
  ...
  return sendEmail({
    to: opts.email,
    subject: "Atualização sobre sua solicitação de parceria — SOS Pet",
    html,
