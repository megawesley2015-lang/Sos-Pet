...
import { PawPrint, Plus, ClipboardList, Filter, ShieldAlert } from "lucide-react";
...
export default async function OngPetsPage({ searchParams }: PageProps) {
  ...
  if (!isAdmin) baseQuery = baseQuery.eq("ong_id", user.id);
      ...
      {isAdmin && (
        <div className="flex items-center gap-3 rounded-xl border border-brand-500/40 bg-brand-500/10 px-4 py-3">
          ...
          <p className="text-sm text-fg">
            <strong className="text-brand-300">Modo Admin</strong> — exibindo prontuários de <strong>todas as ONGs</strong>.
          </p>
        </div>
      )}
