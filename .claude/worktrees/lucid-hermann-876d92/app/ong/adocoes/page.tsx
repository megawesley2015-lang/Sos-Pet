...
import { Users, Plus, Phone, CheckCircle2, Clock, AlertCircle, ShieldAlert } from "lucide-react";
...
export default async function AdocoesPage() {
  ...
  const { data: adocoesRaw } = isAdmin
    ? await adocoesQuery
    : await adocoesQuery.eq("ong_id", user.id);
  ...
  const petsDispQuery = supabase.from("prontuarios").select("id, pet_id").limit(100);
  const { data: petsDisponiveisRaw } = isAdmin
    ? await petsDispQuery
    : await petsDispQuery.eq("ong_id", user.id);
      ...
      {isAdmin && (
        <div className="flex items-center gap-3 rounded-xl border border-brand-500/40 bg-brand-500/10 px-4 py-3">
          ...
          <p className="text-sm text-fg">
            <strong className="text-brand-300">Modo Admin</strong> — exibindo adoções de <strong>todas as ONGs</strong>.
          </p>
        </div>
      )}
