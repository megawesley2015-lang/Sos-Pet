...
import { getUserWithProfile } from "@/lib/auth/safe";
import { redirect } from "next/navigation";
import Link from "next/link";
import { PawPrint, ClipboardList, Users, Heart, AlertTriangle, Clock, ShieldAlert } from "lucide-react";
...
export default async function OngDashboard() {
  ...
  // Pré-busca IDs de prontuários do escopo correto para usar nas queries de
  // medicações e vacinas (que não têm coluna ong_id direta).
  const pronIdsQuery = supabase.from("prontuarios").select("id");
  const { data: pronIdsRaw } = isAdmin
    ? await pronIdsQuery
    : await pronIdsQuery.eq("ong_id", user.id);
  const pronIds = (pronIdsRaw ?? []).map((p) => p.id);

  // Queries de contagem — sem filtro por ong_id quando admin
  const qProntuarios = (() => {
    const q = supabase.from("prontuarios").select("*", { count: "exact", head: true });
    return isAdmin ? q : q.eq("ong_id", user.id);
  })();

  const qPets = (() => {
    const q = supabase.from("pets").select("*", { count: "exact", head: true }).eq("status", "active");
    return isAdmin ? q : q.eq("owner_id", user.id);
  })();

  const qAdotados = (() => {
    const q = supabase.from("adocoes").select("*", { count: "exact", head: true }).eq("status", "ativo");
    return isAdmin ? q : q.eq("ong_id", user.id);
  })();

  const qMedicacoes = (() => {
    const q = supabase.from("medicacoes").select("*", { count: "exact", head: true }).eq("ativa", true);
    return isAdmin ? q : q.in("prontuario_id", pronIds.length ? pronIds : [""]);
  })();
  ...
  const qVacinas = (() => {
    const q = supabase
      ...
      .lte("proxima_dose", new Date(Date.now() + 30 * 86400_000).toISOString().slice(0, 10));
    return isAdmin ? q : q.in("prontuario_id", pronIds.length ? pronIds : [""]);
  })();
  ...
  ] = await Promise.all([qProntuarios, qPets, qAdotados, qMedicacoes, qVacinas]);

  // Últimos prontuários (sem join para evitar tipo array→objeto)
  const recentesQuery = supabase
    .from("prontuarios")
    .select("id, data_resgate, situacao_saude, pet_id")
    .order("data_resgate", { ascending: false })
    .limit(5);
  const { data: recentesRaw } = isAdmin
    ? await recentesQuery
    : await recentesQuery.eq("ong_id", user.id);
  ...
  const acompQuery = supabase
    ...
    .limit(5);
  const { data: pendentesAcomp } = isAdmin
    ? await acompQuery
    : await acompQuery.eq("ong_id", user.id);
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-fg-muted">
          {isAdmin ? "Métricas globais — todos os abrigos." : "Métricas em tempo real do seu abrigo."}
        </p>
      </div>
      ...
      {isAdmin && (
        <div className="flex items-center gap-3 rounded-xl border border-brand-500/40 bg-brand-500/10 px-4 py-3">
          ...
          <p className="text-sm text-fg">
            <strong className="text-brand-300">Modo Admin</strong> — você está visualizando dados de <strong>todas as ONGs</strong>.
          </p>
        </div>
      )}
